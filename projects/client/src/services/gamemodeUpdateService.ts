...
} from "../types/messages/updateGameModeDataMessage";
...
export class GamemodeUpdateService extends ClientListener {
  ...
  private onCreateActorMessage(event: ConnectionMessage<CreateActorMessage>) {
    ...
    this.controller.once("tick", () => {
      ...
      const myFormModel: FormModel | undefined =
        worldModel.forms[worldModel.playerCharacterFormIdx];
  ...
  private updateGamemodeUpdateFunctions(storageVar: string, functionSources: GamemodeValuePair[]) {
    ...
    for (const propName of Object.keys(functionSourcesRecord)) {
      const result = serverJsVerificationService.verifyServerJs(
      ...
      try {
        (this.sp.storage[storageVar] as any)[propName] = new Function("ctx", result.src);
