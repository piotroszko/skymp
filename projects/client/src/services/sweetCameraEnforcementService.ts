...
export class SweetCameraEnforcementService extends ClientListener {
  ...
  private onCustomPacketMessage2(e: ConnectionMessage<CustomPacketMessage>) {
    ...
    try {
      ...
      if (err instanceof SyntaxError) {
        logError(
          this,
          "Failed to parse custom packet contentJsonDump:",
          e.message.contentJsonDump,
          "Error:",
          err,
        );
        return;
    ...
    this.controller.once("update", () => {
      logTrace(
        this,
        "Trying to invoke anim",
        name,
        "with weaponDrawnAllowed=",
        weaponDrawnAllowed,
        ", furnitureAllowed=",
        furnitureAllowed,
        ", exitAnimName=",
        ...
        ", timeMs=",
        timeMs,
        ", isPlayExitAnimAfterwardsEnabled=",
        isPlayExitAnimAfterwardsEnabled,
        ", parentAnimEventName=",
        parentAnimEventName,
        ", enablePlayerControlsDelayMs=",
        enablePlayerControlsDelayMs,
        ", preferInterruptAnimAsExitAnimTimeMs=",
        preferInterruptAnimAsExitAnimTimeMs,
        ", preventManualInterrupt=",
        preventManualInterrupt,
      );

      const result = this.tryInvokeAnim(name, {
        weaponDrawnAllowed,
        furnitureAllowed,
        exitAnimName,
        interruptAnimName,
  ...
  private onSendAnimationEventLeave(ctx: { animEventName: string; animationSucceeded: boolean }) {
    ...
    if (animLowerCase.startsWith("idle") && !animLowerCase.startsWith("idleforcedefaultstate")) {
      this.controller.once("update", () => {
        ...
        this.currentAnim = {
          name: ctx.animEventName,
          options: null,
          preferInterruptAnimState: null,
        };
  ...
  private exitAnim(options: ExitAnimOptions) {
    ...
    if (options.playExitAnim) {
      ...
      if (options.useInterruptAnimAsExitAnim) {
        ...
        logTrace(
          this,
          `Using interrupt anim as exit anim: ${this.interruptAnimName}. Reason: useInterruptAnimAsExitAnim is true`,
        );
      }
      ...
      if (this.currentAnim?.preferInterruptAnimState?.prefer) {
        ...
        logTrace(
          this,
          `Using interrupt anim as exit anim: ${this.interruptAnimName}. Reason: preferInterruptAnimState.prefer is true`,
        );
      }

      const animEventToSend = useInterruptAnimAsExitAnim
        ? this.interruptAnimName || "IdleStop"
        : this.exitAnimName || "IdleForceDefaultState";
      ...
      const enablePlayerControlsDelaySeconds =
        typeof options.enablePlayerControlsDelayMs === "number" &&
  ...
  private onSendExitAnimationEventLeave(ctx: {
    ...
    if (
      ...
      this.exitAnimEvent &&
      this.optionsCache &&
      ctx.animEventName === this.exitAnimEvent
    ) {
      this.currentAnim = null;
      const enablePlayerControlsDelaySeconds =
        typeof this.optionsCache.enablePlayerControlsDelayMs === "number" &&
    ...
    if (
      ...
      if (this.needsExitingAnim) {
        ...
        if (this.currentAnim?.options) {
          this.exitAnim({
            playExitAnim: true,
            enablePlayerControlsDelayMs: this.currentAnim.options.enablePlayerControlsDelayMs,
          });
        } else {
          this.exitAnim({ playExitAnim: true, enablePlayerControlsDelayMs: undefined });
        }
      ...
      if (this.needsExitingAnim) {
        ...
        if (!intervalMs || Date.now() - this.lastNotificationMoment >= intervalMs) {
          ...
          this.sp.Debug.notification("Press Space to exit the animation");
        }
      }
    }
  ...
  private tryInvokeAnim(
    animEvent: string,
    options: InvokeAnimOptions,
    ...
    if (
      ...
      this.sp.Utility.wait(timeSeconds).then(() => {
        ...
        const isPlayExitAnimAfterwardsEnabled =
          typeof options.isPlayExitAnimAfterwardsEnabled === "boolean"
            ? options.isPlayExitAnimAfterwardsEnabled
            : true;

        this.exitAnim({
          playExitAnim: isPlayExitAnimAfterwardsEnabled,
          enablePlayerControlsDelayMs: options.enablePlayerControlsDelayMs,
        });
      });
    }
    ...
    if (animEvent.toLowerCase() === "idleforcedefaultstate") {
      ...
      const doInvoke = () => {
        ...
        if (
          typeof options.preferInterruptAnimAsExitAnimTimeMs === "number" &&
          ...
          logTrace(
            this,
            `Prefer interrupt anim as exit anim for ${options.preferInterruptAnimAsExitAnimTimeMs} ms`,
          );
      ...
      if (
        typeof options.parentAnimEventName === "string" &&
        this.currentAnimName === options.parentAnimEventName
      ) {
        doInvoke();
      } else if (
        Array.isArray(options.parentAnimEventName) &&
        options.parentAnimEventName.includes(this.currentAnimName)
      ) {
        doInvoke();
