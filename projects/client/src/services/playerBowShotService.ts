...
export class PlayerBowShotService extends ClientListener {
  constructor(
    ...
    this.controller.emitter.on("queryBlockSetInventoryEvent", (e) =>
      this.onQueryBlockSetInventoryEvent(e),
    );
  ...
  private onPlayerCrossbowShot() {
    ...
    const equippedAmmoEntries = getEquipment(actor, 0).inv.entries.filter(
      (entry) => entry.worn && Ammo.from(Game.getFormEx(entry.baseId)),
    );
