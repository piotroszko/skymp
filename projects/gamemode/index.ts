...
import {
  SweetTaffyTimedRewards,
  TimedRewardConfig,
} from "./src/logic/listeners/sweettaffyTimedRewards/SweetTaffyTimedRewards";
...
const err = (index: number, x: unknown, expectedTypeName: string): never => {
  throw new TypeError(
    `The argument with index ${index} has value (${JSON.stringify(x)}) that doesn't meet the requirements of ${expectedTypeName}`,
  );
};
...
const getArray = (args: PapyrusValue[], index: number, type: string): unknown[] => {
  ...
  return Array.isArray(x) && !x.filter((v) => typeof v !== type).length
    ? (x as unknown[])
    : err(index, x, `${type}[]`);
};
...
const printArgs = (args: PapyrusValue[]): string => {
  ...
  for (const pVal of args) {
    out +=
      " " + (typeof pVal === "object" || Array.isArray(pVal) ? JSON.stringify(pVal) : pVal) + ";";
  }
  return out;
};
...
export const getRequiredItemCount = (
  mp: Mp,
  self: null,
  args: PapyrusValue[],
  exchanges: SPExchange[],
...
export const getReturnItemIndex = (
  mp: Mp,
  self: null,
  args: PapyrusValue[],
  exchanges: SPExchange[],
...
export const getReturnItemCount = (
  mp: Mp,
  self: null,
  args: PapyrusValue[],
  exchanges: SPExchange[],
...
declare const mp: Mp;
mp.registerPapyrusFunction("global", "Utility", "RandomInt", (self, args) =>
  randomInt(mp, self, args),
);
...
mp.registerPapyrusFunction("method", "ObjectReference", "MoveTo", (self, args) =>
  moveTo(mp, self, args),
);
mp.registerPapyrusFunction("method", "Actor", "IsDead", (self, args) => isDead(mp, self, args));
mp.registerPapyrusFunction("global", "SweetPie", "SPLog", (self, args) =>
  placeholder(mp, self, args, "SPLog"),
);
...
MpApiInteractor.setup([
  ...
  new SweetTaffyTimedRewards(
    controller,
    mp.getServerSettings().sweetPieRewards as TimedRewardConfig,
  ),
