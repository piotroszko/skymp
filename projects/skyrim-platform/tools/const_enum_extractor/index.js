...
const skyrimPlatformTsPath = path.resolve(
  __dirname,
  "../../src/platform_se/codegen/convert-files/skyrimPlatform.ts",
);
const constEnumApiCppPath = path.resolve(
  __dirname,
  "../../src/platform_se/skyrim_platform/ConstEnumApi.cpp",
);
...
const body = extractConstEnums(tsCode)
  .map(({ name, values }) => {
    const enumObject = `  auto ${name} = Napi::Object::New(env);\n`;

    const setProperty = Object.entries(values)
      .map(([key, value]) => {
        return `  ${name}.Set(${prepareKey(value)}, "${key}");\n`;
      })
      .join("");

    const setPropertyString = Object.entries(values)
      .map(([key, value]) => {
