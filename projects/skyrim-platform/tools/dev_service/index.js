  ...
  if (!fs.existsSync(dir)) {
    while (true)
      try {
        ...
        if (process.platform !== "win32" || e.toString().indexOf("EPERM") === -1) {
          throw e;
        }
      }
  }
...
const watchCallback = (_eventType, fileName) => {
  {
    if (fileName === "touch_Release" || fileName === "touch_Debug") {
      ...
      let cp = (from, targetDir) =>
        writeFileSyncRecursive(path.join(targetDir, getFileName(from)), fs.readFileSync(from));
      ...
      if (process.platform === "win32") {
        ...
        ["chrome_100_percent.pak", "chrome_200_percent.pak", "resources.pak"].forEach((item) => {
          cp(
            path.join(cefDir, "Resources", item),
        ...
        requiredVcpkgDlls.forEach((dll) => {
          cp(binPath(dll), path.join(distDir, "Data/Platform/Distribution/RuntimeDependencies"));
        });
        ...
        nodeEmbedderApiDirs.forEach((dirName) => {
          ...
          const nodeEmbedderApiDir = path.join(
            binaryDir,
            `vcpkg_installed/x64-windows-sp/bin/${dirName}`,
          );
          if (!copiedNodeDlls) {
      ...
      const spDotTsPath2 = path.join(
        sourceDir,
        "src/platform_se/codegen/convert-files/skyrimPlatform.ts",
      );
      let spDotTsPath;
      ...
      cp(spDotTsPath, path.join(distDir, "Data/Platform/Modules"));
      cp(spDotTsPath, path.join(sourceDir, "src/platform_se/codegen/convert-files"));
      // Mirror the codegen output to the published .d.ts so downstream TS workspaces
      // (skymp5-client, etc.) always see the latest API. The CMake sp_codegen target
      // does the same on full builds; this keeps the dev-watch loop in sync too.
      fs.copyFileSync(spDotTsPath, path.join(sourceDir, "index.d.ts"));
